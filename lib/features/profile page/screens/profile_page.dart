...
class ProfilePage extends StatelessWidget {
  ...
  Future<Map<String, dynamic>?> _getUserHealthData(String uid) async {
    final doc =
      ...
      body: SingleChildScrollView(
        child: Padding(
          ...
          child: FutureBuilder<Map<String, dynamic>?>(
            future:
                user != null
                    ? _getUserHealthData(user.uid)
                    : Future.value(null),
            builder: (context, snapshot) {
              final healthData = snapshot.data;
              // Defensive: handle double/int/num and null
              String getDisplayValue(dynamic value, {String? suffix}) {
                if (value == null) {
                  return '--';
                }
                if (value is double) {
                  return '${value.toStringAsFixed(1)}${suffix ?? ''}'; // Format doubles
                }
                if (value is int) {
                  return '${value}${suffix ?? ''}';
                }
                if (value is num) {
                  // Fallback for other numbers like integers as doubles (from Firestore)
                  return '${value.toStringAsFixed(value.toInt() == value ? 0 : 1)}${suffix ?? ''}';
                }
                    ...
                    children: [
                      ...
                      Column(
                        children: [
                          ...
                          SizedBox(height: 5),
                          Text(
                            getDisplayValue(healthData?['age']),
                            style: GoogleFonts.nunito(
                              color: Colors.white,
